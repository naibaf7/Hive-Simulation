...
\subsection{Basic model: Hive simulation}
	...
	\subsection{Empirical data bases}
		...
		\subsubsection{Flower patches and food}
			...
			$v$ is the parameter that governs the effect of hive bees on brood survival, $f$ governs the food effect on brood survival respectively (see section \ref{hiveSimulation}). $f=50~000$ corresponds to 50~000 g of food to start with. The parameter $\phi=\frac{1}{9}$, the adult bee emerging factor is assumed. Those values are extracted from \textit{Khoury's} paper and are empirically tested but not explained any further.\\ Although, the food consumption for adult bees (0.007 g) and brood (0.018 g) is taken from explicitly mentioned data \cite{harbo93}.
					
			As remarked at the beginning of section \ref{hiveSimulation}, we simplify the complicated ageing process (larvae to hive bee) to 12 days. Mortality rate is set to 0.075 and thus slightly lower than the values of \textit{Khoury}. This will be discussed later in section \ref{chap:constantFoodDynamicLaying}.) 
			\\
