...
\subsection{Basic model: Hive simulation}
	...
	\subsection{Empirical data bases}
		...
		\begin{equation}\label{eq:foodPerFlight}
					W = \frac{M \frac{kg}{d}}{C}*1000 \frac{g}{kg} = Weight~per~bee~and~flower
		\end{equation}
		Where M is a local certain height in the graph (fig.~\ref{fig:seasonalFlowers}) and C is the bee count of the hive. Thus we can take hive weight change per day as an indicator for the flower quality at any given day of the year. 
		
		
			\begin{figure}
					\centering
					\scalebox{.75}{\includegraphics{data/seasonal_flowers.eps}}
