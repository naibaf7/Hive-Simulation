...
\subsection{Basic model: Hive simulation}
	...
	\subsection{Advanced model: Environment simulation}
		...
		\subsubsection{Bees' working states}
			...
			\begin{itemize}
				...
				\item The forager flies to the patch and, once reached, collects as much food as the flowers in the patch allow (see chapter \ref{chap:mapFlowerPatchQuality}).
		...
		\subsubsection{Foragers' distribution across flower patches}
			...
			Of course, on the implementation side, we have to keep track which vector index $i \in [1,n_w] \subsetneq \N$ belongs to which path element. The path a bee chooses will be copied to the bee's memory.\\

			A path for a certain patch can get smaller by optimization (see chapter \ref{chap:pathOptimization}), thus we can obtain a better relative quality $q_w$. This will lead to higher propagation probability for the same patch. This means more bees will (over time) take the shorter path to any given flower patch, either by optimizing themselves or by looking at waggle dances of patches with shorter paths.\\
		...
		\subsubsection{Food collection}
			With every flight, a bee can gather up to $f_b = \min(b_w,f_{b,max})$ grams of pollen and nectar, where $f_{b,max}$ is the physical top limit of the bee. $b_w$ is derived from $f_{b,f}$ (see chapter \ref{chap:flowerPatchesAndFood}) by multiplying $f_{b,f}$ with the growing quality of a certain patch on day $t$, denoted $q_{p,t} \in (0,1]$. The daily food sum is increased every time a bee reaches the hive:
			
			\begin{equation}\label{eq:foodChangeDailyComputation}
				f_{d,t} = f_{d,t} + f_b \cdot F_{b,t} = f_{d,t} + \min(f_{b,f} \cdot q_{p,t},f_{b,max}) \cdot F_{b,t}
			\end{equation}
	...
	\subsection{Empirical data bases}
		...
		\subsubsection{Flower patches and food}
			...
			Our bee number input is fully build on \textit{Khoury's} model \cite{khoury13}. He used 16000 hive bees and 8000 forager bees which seems to be the right order of magnitude.
			
			The efficiency by which the food is delivered to the brood by the hive bees, $v$, is according to \textit{Khoury} set to 5000 and indicates, that about half of the eggs die before pupation. $f=500$ means, that there is 500 g of food to start with. 
			
			As remarked in the section \ref{hiveSimulation}, we simplify the complicated ageing process (larvae to hive bee) to a 12-day-cycle. On the other hand, mortality rate is 0.075 (compare \ref{subsec:Constant food, dynamic egg laying}.) 
