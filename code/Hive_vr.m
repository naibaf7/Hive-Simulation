classdef Hive_vr < handle
    ...
    properties
        ...
        beemap_foragers;         % Rasterized map of bees (N/10xN/10, where N is the world size)
        beemap_scouts;           % Rasterized map of bees (N/10xN/10, where N is the world size)
        
        beemap_vr_scaling;       % Video scaling / summing up
    ...
    methods
        % Constructor
        function obj = Hive_vr(hive_index, world, Prop)
            obj.prop = Prop;
            ...
            obj.beemap_vr_scaling = 1/4;
            obj.beemap_foragers = Map(0,obj.prop.Sim.world_size_10*obj.beemap_vr_scaling,obj.prop.Sim.world_size_10*obj.beemap_vr_scaling);
            obj.beemap_scouts = Map(0,obj.prop.Sim.world_size_10*obj.beemap_vr_scaling,obj.prop.Sim.world_size_10*obj.beemap_vr_scaling);
            ...
            obj.waggle_count = 0;
             for i = 1:round(obj.F(t_d)*obj.max_scout_percent)
                 ...
                 if(obj.scouts(i).work_mode ~= 0)
                     ...
                     x = ceil(obj.scouts(i).x_pos*obj.beemap_vr_scaling);
                     y = ceil(obj.scouts(i).y_pos*obj.beemap_vr_scaling);
                     obj.beemap_scouts.array(y,x) = obj.beemap_scouts.array(y,x) - 1;
                 end
             end
             for i = 1:obj.max_forager_clusters
                 % Don't record bees that don't work
                 if(obj.foragers(i).work_mode ~= 0)
                     % Add bee record at current position
                     x = ceil(obj.foragers(i).x_pos*obj.beemap_vr_scaling);
                     y = ceil(obj.foragers(i).y_pos*obj.beemap_vr_scaling);
                     obj.beemap_foragers.array(y,x) = obj.beemap_foragers.array(y,x) - obj.cluster_size;
                 end
             end
